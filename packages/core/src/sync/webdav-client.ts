...
export class WebDavClient {
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      try {
        ...
        if (attempt < RETRY_MAX_ATTEMPTS) {
          const delay = RETRY_BASE_DELAY_MS * 2 ** attempt;
          console.warn(
            `[WebDAV] ${method} ${logPath} network error after ${elapsed}ms, retrying in ${delay}ms (attempt ${attempt + 1}/${RETRY_MAX_ATTEMPTS})`,
          );
          await new Promise((resolve) => setTimeout(resolve, delay));
          continue;
        }
  ...
  /** Download a remote file directly to a local path without buffering the body. */
  async getFileToPath(
    path: string,
    localPath: string,
    onProgress?: (loaded: number, total: number) => void,
    ...
    if (!platform.downloadFile) {
      throw new Error("Direct file download is not supported by this platform");
    }

    const url = this.buildUrl(path);
    const logPath = path.startsWith("/") ? path : `/${path}`;
    console.log(`[WebDAV] GET ${logPath} -> ${localPath} (direct file)`);
    const startTime = Date.now();

    await platform.downloadFile(url, localPath, {
      method: "GET",
      headers: { Authorization: this.authHeader },
      allowInsecure: this.allowInsecure,
      timeoutMs: TRANSFER_TIMEOUT_MS,
    ...
    this.hadAuthSuccess = true;
    console.log(
      `[WebDAV] GET ${logPath} direct file completed in ${Date.now() - startTime}ms`,
    );
  }
