...
vi.mock("../../services/platform", () => ({
  getPlatformService: vi.fn(() => mockPlatformService),
}));
...
describe("sync-files", () => {
  beforeEach(() => {
    ...
    mockPlatformService.isMobile = false;
  });
  ...
  describe("syncFiles", () => {
    ...
    it("downloads mobile book files directly to disk instead of buffering in JS memory", async () => {
      mockPlatformService.isMobile = true;
      mockSelect.mockResolvedValue([
        {
          id: "book-1",
          file_path: "books/book-1.epub",
      ...
      const getFileToPathMock = vi.fn().mockResolvedValue(undefined);
      const backend = createMockBackend({
        get: vi.fn().mockResolvedValue(new Uint8Array([1, 2, 3])),
        getFileToPath: getFileToPathMock,
        listDir: vi.fn().mockImplementation(async (path: string) => {
          ...
          if (path === `${REMOTE_BOOKS_ROOT}/Test Book-book-1`) {
            return [
              {
                name: "Test Book.epub",
                path: `${REMOTE_BOOKS_ROOT}/Test Book-book-1/Test Book.epub`,
      ...
      expect(result.filesDownloaded).toBe(1);
      expect(getFileToPathMock).toHaveBeenCalledWith(
        `${REMOTE_BOOKS_ROOT}/Test Book-book-1/Test Book.epub`,
        expect.stringContaining("/tmp/readany-sync-"),
        undefined,
      );
    ...
    it("does not fall back to buffered downloads on mobile when direct file download is unavailable", async () => {
      mockPlatformService.isMobile = true;
      mockSelect.mockResolvedValue([
        {
          id: "book-1",
      ...
      expect(result.filesDownloadFailed).toBe(1);
      expect(backend.get).not.toHaveBeenCalled();
    });
