...
const MOBILE_FILE_TRANSFER_CONCURRENCY = 3;
...
function isMobileSyncPlatform(): boolean {
  try {
...
function getFileTransferConcurrency(fallback: number): number {
  return isMobileSyncPlatform() ? MOBILE_FILE_TRANSFER_CONCURRENCY : fallback;
}

function isDirectDownloadUnsupported(error: unknown): boolean {
  const msg = (error as { message?: string })?.message ?? String(error);
  return /direct file download|not supported|unsupported|downloadFile/i.test(msg);
}
...
export async function syncFiles(
  ...
  if (migrationTasks.length > 0) {
    await parallelLimit(migrationTasks, getFileTransferConcurrency(MIGRATION_CONCURRENCY));
  }
  ...
  if (uploadTasks.length > 0) {
    ...
    const uploadResults = await parallelLimit(
      tasksWithProgress,
      getFileTransferConcurrency(UPLOAD_CONCURRENCY),
    );
  ...
  if (downloadTasks.length > 0) {
    ...
    const downloadResults = await parallelLimit(
      tasksWithProgress,
      getFileTransferConcurrency(DOWNLOAD_CONCURRENCY),
    );
...
async function makeTempDownloadPath(localPath: string): Promise<string> {
  const adapter = getSyncAdapter();
  const tempDir = await adapter.getTempDir();
  const fileName = getFileName(localPath) || "download.tmp";
  const unique = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
  return adapter.joinPath(tempDir, `readany-sync-${unique}-${fileName}`);
}
...
async function downloadRemoteFileToPath(
  ...
  const adapter = getSyncAdapter();
  const mobile = isMobileSyncPlatform();
  const localDir = getDirName(localPath);

  if (backend.getFileToPath) {
    const tempPath = await makeTempDownloadPath(localPath);
    try {
      const tempDir = getDirName(tempPath);
      if (tempDir) await adapter.ensureDir(tempDir);
      await backend.getFileToPath(remotePath, tempPath, onProgress);
      if (localDir) await adapter.ensureDir(localDir);
      await adapter.copyFile(tempPath, localPath);
      return undefined;
    } catch (error) {
      if (mobile || !isDirectDownloadUnsupported(error)) {
        throw error;
      }
      console.warn(
        `[Sync] Direct download unsupported for ${remotePath}; falling back to buffered desktop download.`,
      );
    } finally {
      try {
  ...
  if (mobile) {
    throw new Error(
      `Direct file download is required on mobile for ${remotePath}; buffered fallback skipped.`,
    );
  }
...
function buildDownloadFileTask(
  ...
  return async () => {
    ...
    try {
      ...
      const downloadedSize = await downloadRemoteFileToPath(
        backend,
        info.remoteFilePath,
        info.localFilePath,
      );
      ...
      const sizeMB =
        downloadedSize === undefined ? "unknown" : (downloadedSize / 1024 / 1024).toFixed(2);
...
function buildDownloadCoverTask(backend: ISyncBackend, info: BookInfo): () => Promise<boolean> {
  return async () => {
    ...
    try {
      ...
      const sizeKB = downloadedSize === undefined ? "unknown" : (downloadedSize / 1024).toFixed(2);
...
async function cleanupRemoteOrphans(
  ...
  if (tasks.length > 0) {
    ...
    await parallelLimit(tasks, getFileTransferConcurrency(REMOTE_CLEANUP_CONCURRENCY));
  }
}
...
async function cleanupLocalOrphans(
  ...
  if (tasks.length > 0) {
    ...
    await parallelLimit(tasks, getFileTransferConcurrency(5));
  }
}
...
export async function downloadBookFile(
  ...
  try {
    ...
    const downloadCandidate = async (p: string) => {
      await downloadRemoteFileToPath(backend, p, localPath, (loaded, total) => {
    ...
    if (newPath) {
      try {
        await downloadCandidate(newPath);
        downloaded = true;
    ...
    if (!downloaded) {
      try {
        await downloadCandidate(legacyPath);
        downloaded = true;
          ...
          try {
            ...
            try {
              ...
              if (hit) {
                ...
                await downloadCandidate(hitPath);
                downloaded = true;
    ...
    console.log(`[Sync] Book ${bookId} fetched`);
