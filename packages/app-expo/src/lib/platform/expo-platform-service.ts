...
const MOBILE_BUFFERED_DOWNLOAD_LIMIT_BYTES = 20 * 1024 * 1024;
...
export class ExpoPlatformService implements IPlatformService {
  ...
  async downloadFile(url: string, localPath: string, options?: FetchOptions): Promise<void> {
    const { allowInsecure, onDownloadProgress, ...downloadOptions } = options ?? {};
    const effectiveUrl = allowInsecure ? url.replace(/^https:\/\//i, "http://") : url;
    const dirPath = localPath.replace(/\/[^/]+$/, "");
    ...
    const destinationFile = new File(localPath);
    onDownloadProgress?.(0, 0);
    await File.downloadFileAsync(effectiveUrl, destinationFile, {
      headers: headersToRecord(downloadOptions.headers),
      idempotent: true,
    });
    const size = destinationFile.exists ? (destinationFile.size ?? 0) : 0;
    onDownloadProgress?.(size, size);
  }
      ...
      const finalizeResponse = () => {
        ...
        try {
          ...
          if (
            ...
            contentLength > MOBILE_BUFFERED_DOWNLOAD_LIMIT_BYTES
          ) {
            reject(
              new Error(
                `Buffered mobile download refused (${Math.round(contentLength / 1024 / 1024)} MB). Use direct file download instead.`,
              ),
            );
            return;
          }
